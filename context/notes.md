...
| `notes/grading-rubric.md` | Why grades (SS–F) instead of fit/no-fit, critical dimensions (career ceiling, breadth, company signal, sponsorship), rubric feeds career coaching, question-first rewrite (session 4) replacing dimension-weighted scoring |
...
| `notes/db-maintenance.md` | Clean DB operation (remove F/C grades, stale >14d jobs, no auto-archive of C companies), ATS verification sweeps, re-evaluation triggers on profile changes |
