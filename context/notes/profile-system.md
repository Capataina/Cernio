...
| **Cernio-native, off-limits** | `preferences.toml`, `career-goals.md`, `portfolio-gaps.md` | `preferences.toml` is hand-edited mechanical-only config (machine-read by `src/config.rs`); `career-goals.md` is the strategic frame (8 active lanes, role-truth-at-hire, sponsor-only universe, ethical exclusions, Tier 1/2/3 locations) — absorbed the content from the now-retired `preferences.toml [hard]`/`[soft]` sections in May 2026; `portfolio-gaps.md` is written by the `check-integrity` skill |
