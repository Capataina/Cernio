...
**Read the ENTIRE profile — not just projects.** The per-project files in `profile/projects/` show what the candidate CAN build, but `interests.md` shows what the candidate WANTS to engage with. These are not always the same — the candidate might have deep interests in health, science, or domains they haven't built projects in yet. `preferences.toml` captures sector preferences. `cover-letter.md` reveals motivations and framing. All of these inform engagement assessment.
...
| **Technical alignment** | Does the company's core work involve problems that match the portfolio? Map the per-project files in `profile/projects/` and `skills.md` against the company's engineering work. **Weight by project status** — alignment with active or complete projects is strong evidence; alignment only with paused/dormant or abandoned projects is weaker. The status comes from each per-project file's frontmatter. | The portfolio is the primary evidence of capability. A company whose problems resemble the candidate's active or completed work is a company where the portfolio converts in interviews. Alignment with abandoned or paused projects is not the same signal. |
