...
- **Hard Q5 exclusions** (role-type per the role-truth-at-hire hard rule in `profile/career-goals.md`, location outside the tier table in `profile/career-goals.md` §Soft preferences, sector on the ethical-exclusions list in `profile/career-goals.md` — gambling / adtech / consumer-crypto). Q1 prose names which exclusion fires and quotes the JD text that establishes it.
