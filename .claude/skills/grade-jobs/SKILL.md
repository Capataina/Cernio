...
- [ ] **Concept-fit citation present** — SS / S / A fit assessments cite at least one specific engineering paradigm from `profile/skills.md` §Concepts and Domains that the role's description demands (e.g. "low-latency", "distributed systems", "performance-critical", "ML infrastructure", "lock-free", "observability"). Stack-fit alone (naming languages or frameworks) is insufficient when the role's selection signal is paradigm-based. Per `references/grading-rubric.md` §Q3 decomposition: Concept-fit is load-bearing, Stack-fit is the tiebreaker.
