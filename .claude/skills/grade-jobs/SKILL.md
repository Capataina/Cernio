...
> 1a. **Job lane ≠ company lane**: the job's lane is classified from the role's function (title + JD), **NOT inherited from the parent company's lane**. A Platform Engineer at Monzo is `systems-infra` even though Monzo is `fintech`. An ML Researcher at Goldman is `ai-ml` even though Goldman is `bank-strats`. A Compiler Engineer at Citadel is `systems-infra` even though Citadel is `hft`. The parent company's lane is read for *context* (pinnacle position via Q3) and is used as a fallback **only** when title + JD are too thin to classify the role function on their own (e.g. a bare "Software Engineer" listing with a two-line description and nothing else). When in doubt — classify from the role, not the firm. See `references/lane-divergence-examples.md` for the worked-example table.
