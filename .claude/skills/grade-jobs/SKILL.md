...
- [ ] **SS / S / A assessments make Q1 reasoning visible alongside Q2 reasoning** — when the role is at a brand-name or otherwise reputable firm, the assessment names the firm's hiring-pattern signal (graduate pipeline volume, university-acceptance breadth, screening shape) explicitly, so the realism reasoning is auditable. Conflating reputation (Q2) with selectivity (Q1) without naming the distinction fails this item per `references/grading-rubric.md` §"Common Grading Errors / Grade inflation from prestige".
