...
- **Status**: In Progress
- **Summary**: Real-time brain-inspired learning system built around a custom 2D top-down racing environment, exploring whether a learning system built from scratch using local plasticity rules can acquire driving behaviour in real time without backpropagation, gradients, or any external ML framework.
- **Your role**: Solo. Designs the simulation environment, the sparse neural graph, and every learning rule.
- **Technical highlights**: Engineered around a single research question: can a system that mimics how the brain actually learns gradually acquire driving behaviour and be watched doing it? Every learning mechanism is implemented from first principles in safe Rust on top of a Bevy simulation. The brain is a sparse neural graph with fixed sensorimotor I/O and a plastic hidden topology. Each synapse maintains both a weight and an eligibility trace that accumulates recent correlation between presynaptic and postsynaptic activity, enabling delayed credit assignment without gradients. A scalar reward prediction error acts as a dopamine-like neuromodulatory signal that gates weight consolidation: eligibility marks which synapses participated, the prediction error decides whether that participation should stick. Structural plasticity rules add and prune synapses under bounded fan-in/fan-out constraints to reallocate capacity over time. The environment is a deterministic, fixed-timestep 2D car physics simulation with raycast distance sensors, centerline-projected progress signals, and full debug overlays for sensors, geometry, and learning telemetry. Sequencing is deliberately staged: a deterministic sandbox is built and instrumented first, then a minimal A2C baseline validates that the task is autonomously learnable, then biological plasticity replaces gradients, then ablations isolate causal mechanisms, then spiking neuron dynamics with STDP-style traces are introduced, then structural growth and pruning, then multi-track generalisation, then offline replay and consolidation. Explicitly excludes backpropagation, NEAT, evolution strategies, and any PyTorch/TensorFlow/JAX dependency; the entire learning machinery is hand-built.
